...
截至当前 `w7panel` 分支，性能优化优先级 #1-#8 均已完成代码实现与本地验证，其中 #2、#5 和 #8 已补充任务相关的测试集群验收：

| 编号 | 已完成任务 | 主要改动 | 验证状态 |
|---|---|---|---|
| 1 | xattr allowlist 判断前置 | `setxattr` / `fsetxattr` 先读取 xattr name 并判断是否属于 Sysbox 处理范围，非目标 xattr 直接 continue，避免读取 value 与额外分配 | `go test ./seccomp` 通过 |
| 2 | passthrough cache miss 锁外执行，并加 singleflight | 容器锁内只做 cache get/set，nsenter 与宿主 I/O 移到锁外；同 key cache miss 合并，降低并发击穿 | 本地测试通过；测试集群新建 sysbox pod 验收通过 |
| 3 | `readOnlyResource` / `procUptime` snapshot map 过期清理 | 写入 snapshot 时机会式清理过期 entry，降低长期 container churn 下 map 增长风险 | `go test ./handler/implementations -run 'Test(ReadOnlyResourceSnapshot|ProcUptimeSnapshot|ProcUptimeNonZeroOffset)' -race -shuffle=on -count=1`、`go test ./handler/implementations -race -count=1`、`go test ./... -count=1`、`make` 通过 |
| 4 | openat2 正常成功路径 Info 日志降 Debug | 将 openat2 success-path 统计日志从 Info 降到 Debug，避免高频正常路径刷日志 | `go build ./...` 通过；集群日志确认 openat2 stats 日志为 0 |
| 5 | process attribute `(pid,starttime)` 短缓存 | `ProcessCreate()` 保持轻量，属性在 `init()` 懒加载；按 `(pid,starttime)` 复用 uid/gid/groups/root/cwd 快照，避免共享可变 process/capability 对象 | `go test -race -shuffle=on -count=1 ./...` 通过；测试集群新建 sysbox pod 验收通过 |
| 6 | seccomp notification in-flight limiter 与 pidTracker 清理 | 新增全局 seccomp notification in-flight limiter（128），复用 pidTracker 保持同 pid 串行；移除不安全 scheduler wait 语义 | limiter 并发/释放/同 pid 串行/不同 pid 并发测试和 pidTracker cleanup 测试通过；`go test -race -shuffle=on -count=1 ./...`、`go build ./...`、`git diff --check` 通过 |
| 7 | `/proc/loadavg` sampler 缓存 pid namespace、增加采样预算 | 将 loadavg 逻辑拆分为 `procLoadavg*.go`；pid namespace init-pid 查找增加缓存与 4096 项扫描预算；cache hit 前校验 pidns 与 namespace PID，避免 stale PID 复用 | 缓存命中避免扫描、stale 命中重扫、预算传递测试通过；`go test -race -shuffle=on -count=1 ./...`、`go build ./...`、`git diff --check` 通过 |
| 8 | sysbox-mgr rsync/chown 指标与并发限制 | 新增 `lifecycleIO` limiter，统一限制 rsync 与 recursive chown 并发（默认 2）；记录 started/succeeded/failed、in-queue、duration、last target/error、max observed queue，并通过 Debug 日志输出 stats snapshot | `go test ./lifecycleIO`、`go test ./volMgr -run '^$'`、`go test ./... -run '^$'`、`go build ./...`、`git diff --check` 通过；测试集群重启风暴 + 正常 pod 并发启动验收见下 |

### 全量安装替换后的集群验收

本次验收按集群规范执行，不再只替换单个组件。测试前全量构建并安装替换：

| 组件 | 构建产物 | 构建 commit |
|---|---|---|
| `sysbox-fs` | `sysbox-fs/build/sysbox-fs` | `713cfc3cc31fde68cbbe9066301997012f521283` |
| `sysbox-mgr` | `sysbox-mgr/build/sysbox-mgr` | `a38c175137982b9a51e9fa78b10f2363837aa8d3` |
| `sysbox-runc` | `sysbox-runc/build/amd64/sysbox-runc` | `87837d347ca06167d8d2c0a69817894a88ac1289` |

三个构建产物均复制到 `default/sysbox-debug-tools:/tmp/`，在宿主 `/host/usr/bin/` 中备份原二进制后执行 `install -m 0755` 全量替换。随后重启 `sysbox-fs` 与 `sysbox-mgr`，`systemctl is-active sysbox-fs` 和 `systemctl is-active sysbox-mgr` 均返回 `active`。

安装后按规范重建全部 `runtimeClassName: sysbox-runc` 的 Deployment pod：

- `default/rancherk3s-pgaytddm`：rollout 成功
- `default/sysboxnginx-jiihxewl`：rollout 成功
- `k3k-console-164315/k3k-ckm-cuvfh-server`：rollout 成功
- `k3k-console-164315/k3k-ckm-ks352-server`：rollout 成功
- `k3k-console-164315/k3k-ckm-0fhst-server`：仍为 `CrashLoopBackOff`，与此前已知 workload 状态一致
- `k3k-console-164315/k3k-ckm-kq2cs-server`：仍为 `CrashLoopBackOff`，与此前已知 workload 状态一致

`sysbox-fs` 重启后、旧 sysbox pod 重建完成前的安装窗口（约 `20:01-20:02 UTC`）出现过 `Received seccompNotifMsg generated by unknown container`，符合旧容器尚未重新注册的预期风险；因此正式验收从 `2026-06-27 20:07:59 UTC` 开始，只使用重建后的新 sysbox pod 判定。

#### proc / passthrough 回归

专用测试命名空间为 `sysbox-fulltest`，新建 `runtimeClassName: sysbox-runc`、`hostUsers: false`、`emptyDir` 的 `proc-probe` pod，并在该 pod 内执行：

- proc 资源视图循环 200 次：`cat /proc/meminfo`、`cat /proc/stat`、`cat /proc/loadavg`、`cat /sys/devices/system/cpu/online`、`free`
- passthrough 循环 200 次：`cat /proc/sys/kernel/hostname`、`ls /proc/sys/net/ipv4`、`ls /sys/kernel`

两组循环均成功完成，分别输出 `proc-resource-loop-ok` 和 `passthrough-loop-ok`。

#### sysbox-mgr lifecycle IO 并发限制测试

该测试针对 #8 的实际风险面：一个使用 `sysbox-runc` 的 pod 频繁重启时，是否会因为全局 lifecycle IO limiter 拖慢其他 sysbox pod 启动。

专用测试命名空间为 `sysbox-fulltest`，测试 pod 均使用：

- `runtimeClassName: sysbox-runc`
- `hostUsers: false`
- `emptyDir` 挂载
- 节点已缓存的 sysbox systemd 镜像，避免镜像拉取干扰启动耗时

测试数据：

| 场景 | 正常 pod 数 | 干扰 pod | Ready 总耗时 |
|---|---:|---|---:|
| 基线并发启动 | 5 | 无 | 4s |
| 并发启动第 1 轮 | 5 | `noisy-restart` 写入 2000 个小文件后退出，触发重启 | 4s |
| 并发启动第 2 轮 | 5 | 同上 | 3s |
| 并发启动第 3 轮 | 5 | 同上 | 4s |
| 并发启动第 4 轮 | 5 | 同上 | 4s |
...
- `sysbox-fulltest` 中正常 pod 均完成 `Scheduled -> Pulled -> Created -> Started`，未出现 `FailedCreate`、sandbox 创建失败或 timeout。
- `noisy-restart` 仅出现预期的 `BackOff`。
- `journalctl -u sysbox-mgr -u sysbox-fs --since '2026-06-27 20:07:59 UTC'` 过滤 `panic`、`fatal`、`OOM`、`ENOMEM`、`unsafe procfs`、`thread-self`、`Unexpected error during NotifReceive`、`unknown container`、`timeout`、`failed`、`error`、`openat2 seccomp stats` 无匹配输出。
- 测试完成后已删除 `sysbox-fulltest` namespace。

结论：在全量替换 `sysbox-fs`、`sysbox-mgr`、`sysbox-runc` 并重建现有 sysbox-runc pod 后，#1-#8 覆盖的 proc、passthrough、seccomp 日志与 lifecycle IO 基础风险面均通过本次集群验收。当前单节点测试集群、当前镜像和 `emptyDir` 负载下，一个频繁重启的 sysbox pod 没有造成其他 sysbox pod 启动耗时上升；正常 pod 并发启动耗时保持在 3-4 秒，与 4 秒基线持平。该结论覆盖轻中等生命周期 IO 干扰场景；若要评估极端大目录或 PVC/bind mount 场景，还需要在 Debug 日志或指标采集开启后，用更大的目录树与 10-20 个并发正常 pod 做补充压测。
...
7. loadavg sampler 缓存 pid namespace、增加采样预算。**已实现：先将 oversized `procResources.go` 中的 loadavg 逻辑拆分为 `procLoadavg*.go` 职责文件，再为 pid namespace init-pid 查找增加缓存与 4096 项扫描预算；缓存命中前会校验 cached init pid 仍属于同一 pid namespace 且 namespace PID 仍为 1，避免长期运行后的 stale PID 复用；新增缓存命中避免扫描、stale 命中重扫、预算传递测试，`go test -race -shuffle=on -count=1 ./...`、`go build ./...`、`git diff --check` 通过。**
8. sysbox-mgr rsync/chown 加指标和并发限制。**已实现代码侧生命周期 IO gate：新增 `lifecycleIO` limiter 统一限制 rsync 与 recursive chown 并发（默认 2），记录 started/succeeded/failed、in-queue、duration、last target/error、max observed queue，并在 rsync/chown 完成后通过 Debug 日志输出 stats snapshot；`volMgr` rsync 逻辑从 oversized `volMgr.go` 拆到 `rsync.go`，rootfs/mount recursive chown 调用点走同一 limiter；`go test ./lifecycleIO`、`go test ./volMgr -run '^$'`、`go test ./... -run '^$'`、`go build ./...`、`git diff --check` 通过。完整 `go test ./volMgr` 当前环境因缺少 `rsync` 二进制失败，需在安装 rsync 的验收环境重跑。**
