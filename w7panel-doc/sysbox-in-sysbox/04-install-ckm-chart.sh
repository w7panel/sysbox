...
l1_kubectl -n "$CHART_NAMESPACE" delete daemonset w7panel-sysbox-installer --ignore-not-found >/dev/null
l1_kubectl get runtimeclass sysbox-runc-lite -o jsonpath='{.handler}{"\n"}' | grep -qx sysbox-runc-lite
SYSBOX_RUNC_LITE_BINARY="${SYSBOX_RUNC_LITE_BINARY:-$REPO_DIR/sysbox-runc-lite/build/amd64/sysbox-runc-lite}"
[ -x "$SYSBOX_RUNC_LITE_BINARY" ] || die "sysbox-runc-lite binary missing: $SYSBOX_RUNC_LITE_BINARY (build it with make sysbox-runc-lite)"
log 'installing sysbox-runc-lite into the persistent CKM K3s data volume'
base64 -w0 "$SYSBOX_RUNC_LITE_BINARY" | outer_kubectl -n "$OUTER_NAMESPACE" exec -i "$L1_POD" -c "$L1_CONTAINER" -- sh -c \
  'base64 -d > /var/lib/rancher/k3s/sysbox-runc-lite && chmod 0755 /var/lib/rancher/k3s/sysbox-runc-lite'
runtime_config='[plugins.'"'"'io.containerd.cri.v1.runtime'"'"'.containerd.runtimes.sysbox-runc-lite]
...
l1_exec sh -c "mkdir -p /var/lib/rancher/k3s/agent/etc/containerd/config-v3.toml.d; printf '%s' '$runtime_config_b64' | base64 -d > /var/lib/rancher/k3s/agent/etc/containerd/config-v3.toml.d/sysbox-runc-lite.toml"
l1_kubectl -n "$CHART_NAMESPACE" rollout status deployment/w7panel-sysbox-admission --timeout=180s
log 'sysbox-runc-lite is persisted; restart the CKM server during the normal rollout to load the handler'
