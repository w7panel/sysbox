...
select_existing_ckm() {
  ...
  log "selected existing CKM ${CKM_NAMESPACE}/${CKM_NAME} selector=${CKM_SELECTOR} (inner Sysbox disabled)"
}
...
l1_rootfs_exec() {
  ...
  [ -n "$container_id" ] || die "container ID is unavailable for $pod/$container"
  l1_exec sh -ec '
    cid="$1"; script="$2"; target=""
    for f in /proc/[0-9]*/cgroup; do
      pid=${f#/proc/}; pid=${pid%/cgroup}
      grep -q "$cid" "$f" 2>/dev/null || continue
      target="$pid"; break
    done
    [ -n "$target" ] || { echo "container process not found: $cid" >&2; exit 1; }
    exec nsenter -t "$target" -m -r"/proc/$target/root" -- sh -ec "$script"
  ' sh "$container_id" "$script"
}
check_common() {
  need_cmd kubectl
  need_cmd helm
