...
- /proc/swaps 已支持明确 cgroup swap limit/current、v2 max 向父层查找有效限制、v1 memsw 总量口径和 v1 swappiness=0 处理；无有效限制时不展示宿主 SwapTotal，且没有 LXCFS_SWAP_ON 开关和 swapon/swapoff 虚拟化。宿主 SwapTotal 为 0 时的封顶语义与 LXCFS 仍有差异。
