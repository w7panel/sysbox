...
| /proc/swaps | 在 proc_fuse.c 的 proc_swaps_read() 中结合 swap 配置、memory+swap limit、swappiness 输出虚拟 swap 行，并按宿主 SwapTotal 封顶。 | 当前输出 header；只有从 cgroup 读到明确 swap limit/current 时才输出一条 virtual swap。cgroup v2 使用 memory.swap.max/current；cgroup v1 使用 memory.memsw.limit-memory.limit 作为 swap 总量，并用 memory.memsw.usage-memory.usage 作为已用量；unlimited/max 不展示为宿主 SwapTotal。 | Sysbox 保守隐藏宿主 swap：无明确 cgroup swap 限制或 swap 为 unlimited/max 时继续只输出 header。差异是 Sysbox 没有 LXCFS 的全局 LXCFS_SWAP_ON 配置开关，也不接管 swapon/swapoff。 |
...
- /proc/swaps 已支持明确 cgroup swap limit/current 和 v1 swappiness=0 处理；unlimited/max 不展示为宿主 SwapTotal，且没有 LXCFS_SWAP_ON 开关和 swapon/swapoff 虚拟化。
