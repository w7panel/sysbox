...
| /proc/stat | 在 proc_fuse.c 的 proc_stat_read() 和 proc_cpuview.c 的 cpuview_proc_stat() 中处理。它会读取 cpuacct/cpuset，维护 cgroup 级历史增量，生成更真实的 per-cpu 统计。 | 当前基于容器 uptime、可见 CPU 数、cgroup CPU usage 初始化 cpu/cpuN 行；后续按 cgroup CPU usage delta、宿主 per-cpu 权重和 per-container 状态缓存累计增量，保留非 CPU 行并把 btime 改成容器创建时间。 | 已对齐“需要历史状态避免每次静态估算”的方向，但 LXCFS 依赖 cpuacct.usage_all/cpuview、cpuset 物理 CPU 映射和 CFS quota 逻辑；Sysbox 使用 Go 全局状态与 cgroup v2 cpu.stat/v1 cpuacct 数据近似分摊，精度和 CPU ID 语义仍不完全等同 LXCFS。 |
| /proc/swaps | 在 proc_fuse.c 的 proc_swaps_read() 中结合 swap 配置、memory+swap limit、swappiness 输出虚拟 swap 行，并按宿主 SwapTotal 封顶。 | 当前输出 header；如果能从 cgroup 读到明确 swap limit/current，则输出一条 virtual swap。cgroup v2 支持 memory.swap.max/current；cgroup v1 支持 memory.memsw.*，并按宿主 SwapTotal 封顶、swappiness=0 时只显示已用量。 | 已对齐 LXCFS 的主要输出语义。差异是 Sysbox 没有 LXCFS 的全局 LXCFS_SWAP_ON 配置开关，也不接管 swapon/swapoff；无可靠 cgroup swap 数据时继续只输出 header。 |
...
| /proc/pressure/io | 在 proc_fuse.c 的 proc_pressure_read() 中读取 cgroup pressure 文件；proc_write/proc_poll 还支持 PSI trigger write/poll，其中 poll 受 LXCFS 启动选项控制。 | 当前读取 cgroup v2 io.pressure；失败时回退 /proc/pressure/io；补齐了换行和 snapshot 读取。 | Sysbox 目前只实现 read 视图，没有实现 LXCFS 的 PSI trigger write/poll 行为。 |
| /proc/pressure/cpu | 同 /proc/pressure/io，读取 cpu.pressure，并支持 PSI trigger write/poll。 | 当前读取 cgroup v2 cpu.pressure；失败时回退 /proc/pressure/cpu。 | 差异同上：只实现读取，没有实现 PSI 事件触发器写入和 poll。 |
| /proc/pressure/memory | 同 /proc/pressure/io，读取 memory.pressure，并支持 PSI trigger write/poll。 | 当前读取 cgroup v2 memory.pressure；失败时回退 /proc/pressure/memory。 | 差异同上：只实现读取，没有实现 PSI 事件触发器写入和 poll。 |
...
- /proc/swaps 已支持 cgroup swap limit/current、宿主 SwapTotal 封顶和 v1 swappiness=0 处理，但没有 LXCFS_SWAP_ON 开关和 swapon/swapoff 虚拟化。
