...
func TestBuildMutatingWebhookConfiguration_setsExactRulesAndMatchCondition(t *testing.T) {
	...
	require.Equal(t, []admissionv1.OperationType{admissionv1.Create, admissionv1.Update}, entry.Rules[0].Operations)
	require.Equal(t, []string{"apps"}, entry.Rules[0].Rule.APIGroups)
	require.Equal(t, []string{"v1"}, entry.Rules[0].Rule.APIVersions)
	require.Equal(t, []string{"deployments", "statefulsets", "daemonsets"}, entry.Rules[0].Rule.Resources)
	require.Equal(t, []admissionv1.OperationType{admissionv1.Create, admissionv1.Update}, entry.Rules[1].Operations)
	require.Equal(t, []string{"batch"}, entry.Rules[1].Rule.APIGroups)
	require.Equal(t, []string{"v1"}, entry.Rules[1].Rule.APIVersions)
	require.Equal(t, []string{"jobs", "cronjobs"}, entry.Rules[1].Rule.Resources)
	require.Equal(t, []admissionv1.OperationType{admissionv1.Create, admissionv1.Update}, entry.Rules[2].Operations)
	require.Equal(t, []string{""}, entry.Rules[2].Rule.APIGroups)
	require.Equal(t, []string{"v1"}, entry.Rules[2].Rule.APIVersions)
	require.Equal(t, []string{"pods"}, entry.Rules[2].Rule.Resources)
